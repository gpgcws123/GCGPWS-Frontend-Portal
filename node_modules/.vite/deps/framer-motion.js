...
function useMotionValueEvent(value, event, callback) {
  (0, import_react11.useInsertionEffect)(() => value.on(event, callback), [value, event, callback]);
}
...
function useScroll({ container, target, layoutEffect = true, ...options } = {}) {
  ...
  const useLifecycleEffect = layoutEffect ? useIsomorphicLayoutEffect : import_react12.useEffect;
  useLifecycleEffect(() => {
...
function useMotionValue(initial) {
  ...
  const { isStatic } = (0, import_react13.useContext)(MotionConfigContext);
  if (isStatic) {
    const [, setLatest] = (0, import_react13.useState)(initial);
    (0, import_react13.useEffect)(() => value.on("change", setLatest), []);
  }
  return value;
}
...
function useSpring(source, config = {}) {
  ...
  const activeSpringAnimation = (0, import_react14.useRef)(null);
  const value = useMotionValue(isMotionValue(source) ? toNumber(source.get()) : source);
  const latestValue = (0, import_react14.useRef)(value.get());
  const latestSetter = (0, import_react14.useRef)(() => {
  });
...
function useCycle(...items) {
  const index = (0, import_react21.useRef)(0);
  const [item, setItem] = (0, import_react21.useState)(items[index.current]);
  const runCycle = (0, import_react21.useCallback)(
    (next) => {
...
function useInView(ref, { root, margin, amount, once = false } = {}) {
  const [isInView, setInView] = (0, import_react22.useState)(false);
...
function useInstantTransition() {
  ...
  const unlockOnFrameRef = (0, import_react23.useRef)(-1);
